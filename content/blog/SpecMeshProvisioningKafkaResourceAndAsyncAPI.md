...
Within each section, we have a Kafka configuration section and the schema related to it. The schema captures the data format of the event set to be published. Hence, the ID serves multiple purposes, with the key purpose being the principle. This allows for clear ownership when any resource is provisioned on behalf of this data product. Topics are prefixed with the principle, schemas are published into the schema registry prefixed with a schema principle, and ACLs also use the prefix. ACLs are governed or controlled by the keywords public, private, or protected.
